...
public class ExamOperationHandlers {

    public static void examHandler(HttpServerExchange exchange, Connection conn, String operation_type) {
    ...
    private static void processAndRespond(HttpServerExchange exchange, Connection conn, Map<String, String> values, String operation_type) {
        try {
            ...
            if("add".equalsIgnoreCase(operation_type)){
                response = Exam.addExam(conn, values);
            }else if("select".equalsIgnoreCase(operation_type)){
                if(exchange.getQueryParameters().isEmpty()){
                    response = Exam.Find(conn);
                }else {
                    String id = exchange.getQueryParameters().get("id").getFirst();
                    if (id.isEmpty()) {
                        response = Exam.Find(conn);
                    } else {
                        response = Exam.FindById(conn, Integer.parseInt(id));
                    }
                }
                ...
                response = Exam.updateExam(conn,values,id.getFirst());
                }}
            }else if("teacherExam".equalsIgnoreCase(operation_type)){
                if(exchange.getQueryParameters().isEmpty()){
                    Utils.respondWithError(exchange, "Error ID not ", 500);
                }else{
                    var id = exchange.getQueryParameters().get("id");
                    if(id.isEmpty()){
                        ...
                        response = ExamReport.teacherExam(conn,Integer.parseInt(id.getFirst()));
                    }}
            }
            assert response != null;
            String xmlResponse = Utils.convertJsonArrayToXml(response.getResponse(),"Exams", "Exam");
