...
public class QuestionsOperationHandlers {

    public static void questionsHandler(HttpServerExchange exchange, Connection conn, String operation_type) {
    ...
    private static void processAndRespond(HttpServerExchange exchange, Connection conn, Map<String, String> values, String operation_type) {
        try {
            ...
            if("add".equalsIgnoreCase(operation_type)){
                response = Questions.addQuestions(conn, values);
            }else if("select".equalsIgnoreCase(operation_type)){
                if(exchange.getQueryParameters().isEmpty()){
                    response = Questions.Find(conn);
                }else {
                    String id = exchange.getQueryParameters().get("id").getFirst();
                    if (id.isEmpty()) {
                        response = Questions.Find(conn);
                    } else {
                        response = Questions.FindById(conn, Integer.parseInt(id));
                    }
                }
                ...
                response = Questions.updateQuestions(conn,values,id.getFirst());
                }}
            }
            ...
            if(response.getCode() == 500){
                ...
                String xmlResponse = Utils.convertJsonArrayToXml(response.getResponse(), "Questions", "Question");
