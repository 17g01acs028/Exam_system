...
public class OptionsOperationHandlers {

    public static void optionsHandler(HttpServerExchange exchange, Connection conn, String operation_type) {
    ...
    private static void processAndRespond(HttpServerExchange exchange, Connection conn, Map<String, String> values, String operation_type) {
        try {
            ...
            if("add".equalsIgnoreCase(operation_type)){
                response = Options.addOptions(conn, values);
            }else if("select".equalsIgnoreCase(operation_type)){
                if(exchange.getQueryParameters().isEmpty()){
                    response = Options.Find(conn);
                }else {
                    String id = exchange.getQueryParameters().get("id").getFirst();
                    if (id.isEmpty()) {
                        response = Options.Find(conn);
                    } else {
                        response = Options.FindById(conn, Integer.parseInt(id));
                    }
                }
                ...
                response = Options.updateOptions(conn,values,id.getFirst());
                }}
            }
