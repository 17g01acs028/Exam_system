...
import org.example.reports.StudentMerit;
...
public class ReportOperationHandlers {

    public static void Handler(HttpServerExchange exchange, Connection conn, String operation_type) {
    ...
    private static void processAndRespond(HttpServerExchange exchange, Connection conn, Map<String, String> values, String operation_type) {
        try {
            ...
            if("studentMerit".equalsIgnoreCase(operation_type)){
                ...
                response = ExamReport.studentMerit(conn,5,Integer.parseInt(id.getFirst()));
                }}
            }else  if("studentReport".equalsIgnoreCase(operation_type)){
                if(exchange.getQueryParameters().isEmpty()){
                    Utils.respondWithError(exchange, "Error ID not ", 500);
                }else{
                    var id = exchange.getQueryParameters().get("id");
                    if(id.isEmpty()){
                        ...
                        new StudentReport(conn,"mysql",Integer.parseInt(id.getFirst()));
                    }}
            }
            ...
            if(response.getCode() == 500){
                ...
                String xmlResponse = Utils.convertJsonArrayToXml(response.getResponse(), "Teachers", "Teacher");
                exchange.getResponseHeaders().put(Headers.CONTENT_TYPE, "application/xml");
                exchange.getResponseSender().send(xmlResponse);
            }
        } catch (Exception e) {
            Utils.respondWithError(exchange, "Error processing request because "+e.getMessage(), 500);
        }
    }
