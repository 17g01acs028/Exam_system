...
public class ResponseOperationHandlers {

    public static void responseHandler(HttpServerExchange exchange, Connection conn, String operation_type) {
    ...
    private static void processAndRespond(HttpServerExchange exchange, Connection conn, Map<String, String> values, String operation_type) {
        try {
            ...
            if("add".equalsIgnoreCase(operation_type)){
                response = Responses.addResponses(conn, values);
            }else if("select".equalsIgnoreCase(operation_type)){
                if(exchange.getQueryParameters().isEmpty()){
                    response = Responses.Find(conn);
                }else {
                    String id = exchange.getQueryParameters().get("id").getFirst();
                    if (id.isEmpty()) {
                        response = Responses.Find(conn);
                    } else {
                        response = Responses.FindById(conn, Integer.parseInt(id));
                    }
                }
                ...
                response = Responses.updateResponses(conn,values,id.getFirst());
                }}
            }
            ...
            if(response.getCode() == 500){
                ...
                String xmlResponse = Utils.convertJsonArrayToXml(response.getResponse(), "Responses", "Response");
