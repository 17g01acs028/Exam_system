...
public class PupilOperationHandlers {

    public static void pupilHandler(HttpServerExchange exchange, Connection conn, String operation_type) {
    ...
    private static void processAndRespond(HttpServerExchange exchange, Connection conn, Map<String, String> values, String operation_type) {
        try {
            ...
            if("add".equalsIgnoreCase(operation_type)){
                response = Student.addStudent(conn, values);
            }else if("select".equalsIgnoreCase(operation_type)){

                if(exchange.getQueryParameters().isEmpty()){
                    response = Student.Find(conn);
                }else{
                    String id = exchange.getQueryParameters().get("id").getFirst();
                    if(id.isEmpty()){
                        response = Student.Find(conn);
                    }else {
                        response = Student.FindById(conn,Integer.parseInt(id));
                    }
                }
                ...
                response = Student.updateStudent(conn,values,id.getFirst());
                }}
            }
            ...
            Utils.respondWithError(exchange, "Error processing request because steve "+e, 500);
        }
    }
