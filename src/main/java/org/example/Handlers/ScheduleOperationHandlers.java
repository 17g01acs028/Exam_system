...
import org.example.controllers.Schedule;
...
public class ScheduleOperationHandlers {

    public static void scheduleHandler(HttpServerExchange exchange, Connection conn, String operation_type) {
    ...
    private static void processAndRespond(HttpServerExchange exchange, Connection conn, Map<String, String> values, String operation_type) {
        try {
            ...
            if("add".equalsIgnoreCase(operation_type)){
                response = Schedule.addSchedule(conn, values);
            }else if("select".equalsIgnoreCase(operation_type)){
                if(exchange.getQueryParameters().isEmpty()){
                    response = Schedule.Find(conn);
                }else {
                    String id = exchange.getQueryParameters().get("id").getFirst();
                    if (id.isEmpty()) {
                        response = Schedule.Find(conn);
                    } else {
                        response = Schedule.FindById(conn, Integer.parseInt(id));
                    }
                }
                ...
                response = Schedule.updateSchedule(conn,values,id.getFirst());
                }}
            }
            ...
            if(response.getCode() == 500){
                ...
                String xmlResponse = Utils.convertJsonArrayToXml(response.getResponse(), "Schedules", "Schedule");
