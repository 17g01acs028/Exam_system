...
public class TeacherOperationHandlers {

    public static void teacherHandler(HttpServerExchange exchange, Connection conn, String operation_type) {
    ...
    private static void processAndRespond(HttpServerExchange exchange, Connection conn, Map<String, String> values, String operation_type) {
        try {
            ...
            if("add".equalsIgnoreCase(operation_type)){
                response = Teacher.addTeacher(conn, values);
            }else if("select".equalsIgnoreCase(operation_type)){
                if(exchange.getQueryParameters().isEmpty()){
                    response = Teacher.Find(conn);
                }else {
                    String id = exchange.getQueryParameters().get("id").getFirst();
                    if (id.isEmpty()) {
                        response = Teacher.Find(conn);
                    } else {
                        response = Teacher.FindById(conn, Integer.parseInt(id));
                    }
                }
                ...
                response = Teacher.updateTeacher(conn,values,id.getFirst());
                }}
            }
