...
public class SubjectOperationHandlers {

    public static void subjectHandler(HttpServerExchange exchange, Connection conn, String operation_type) {
    ...
    private static void processAndRespond(HttpServerExchange exchange, Connection conn, Map<String, String> values, String operation_type) {
        try {
            ...
            if("add".equalsIgnoreCase(operation_type)){
                response = Subject.addSubject(conn, values);
            }else if("select".equalsIgnoreCase(operation_type)){
                if(exchange.getQueryParameters().isEmpty()){
                    response = Subject.Find(conn);
                }else {
                    String id = exchange.getQueryParameters().get("id").getFirst();
                    if (id.isEmpty()) {
                        response = Subject.Find(conn);
                    } else {
                        response = Subject.FindById(conn, Integer.parseInt(id));
                    }
                }
                ...
                response = Subject.updateSubject(conn,values,id.getFirst());
                }}
            }
            ...
            if(response.getCode() == 500){
                ...
                String xmlResponse = Utils.convertJsonArrayToXml(response.getResponse(), "Subjects", "Subject");
