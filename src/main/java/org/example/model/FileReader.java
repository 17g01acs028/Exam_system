...
public class FileReader {
    ...
    public String appIp;
    public int aPort;
   public static String key = Utils.key;
    public FileReader(String configFilePath) {
        try {
            ...
            databaseType = xpath.compile("config/database/Database_Type").evaluate(doc);
            String databaseName = xpath.compile("config/database/Database_Name").evaluate(doc);
            String host = xpath.compile("config/database/Host").evaluate(doc);
              port = Integer.parseInt(xpath.compile("config/database/Port").evaluate(doc));
              aPort = Integer.parseInt(xpath.compile("config/application/Port").evaluate(doc));
              appIp =xpath.compile("config/application/Host").evaluate(doc);

            // Check if username and password are encrypted
            XPathExpression usernameExpr = xpath.compile("config/database/Username/@encrypted");
            String usernameEncrypted = usernameExpr.evaluate(doc);

            XPathExpression passwordExpr = xpath.compile("config/database/Password/@encrypted");
            String passwordEncrypted = passwordExpr.evaluate(doc);


            if ("yes".equals(usernameEncrypted)) {
                this.username = decrypt(xpath.compile("config/database/Username").evaluate(doc), key);
            } else {
                this.username = xpath.compile("config/database/Username").evaluate(doc);
                updateConfigFile(configFilePath, "config/database/Username");
            }

            if ("yes".equals(passwordEncrypted)) {
                this.password = decrypt(xpath.compile("config/database/Password").evaluate(doc), key);
            } else {
                this.password = xpath.compile("config/database/Password").evaluate(doc);
                updateConfigFile(configFilePath, "config/database/Password");
            }
    ...
    public String getAppIp(){return  this.appIp;}
    public int getAppPort(){return  this.aPort;}
